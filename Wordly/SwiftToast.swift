...
let HRToastPositionScroll  =   "scroll"
...
extension UIView {
    ...
    //shows toast in tablecell
    func makeToastInCell(message msg: String) {
        self.makeToast(message: msg, duration: HRToastDefaultDuration, position: HRToastPositionCenter)
    }
    //to use for scrolling screens
    func makeToast(message msg: String, height : CGFloat) {
        scrollHeight = height
        debugPrint("CGFloat Scroll Height For Toast : \(scrollHeight)  h: \(height)")
        self.makeToast(message: msg, duration: HRToastDefaultDuration, position: HRToastPositionScroll)
    }
    ...
    // Allataki yorum satırı içindeki değerlerle en altta çıkıyor.
    //Default değeri böyle.
    /*
    ...
    private func centerPointForPosition(position: AnyObject, toast: UIView) -> CGPoint {
        if position is String {
            ...
            debugPrint(" ViewSize ------- height \(self.bounds.size.height)")
            if position.lowercaseString == HRToastPositionTop {
                return CGPointMake(viewSize.width/2, toastSize.height/2 + 70)
            } else if position.lowercaseString == HRToastPositionDefault {
                return CGPointMake(viewSize.width/2, viewSize.height - toastSize.height/2 - 70)
            }
            else if position.lowercaseString == HRToastPositionScroll {
                return CGPointMake(viewSize.width/2, scrollHeight - toastSize.height/2 - 70)
            }
            else if position.lowercaseString == HRToastPositionCenter {
                return CGPointMake(viewSize.width/2, viewSize.height/2)
            }
